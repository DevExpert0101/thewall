...
describe("The Monument canvas", () => {
  ...
  it("shows a tooltip with the sentence when a visitor hovers it", async () => {
    const user = userEvent.setup();
    const entry = sampleMonumentEntry({
      monumentNumber: 1,
      text: "Maybe kindness was the entire point.",
      sentenceSnapshot: "Maybe kindness was the entire point.",
    });
    render(<MonumentIndex catalog={{ entries: [entry], sealedCount: 1, capacity: null, canvas }} />);
    expect(screen.queryByRole("tooltip")).not.toBeInTheDocument();
    await user.hover(screen.getByRole("link", { name: /maybe kindness was the entire point/i }));
    const tip = await screen.findByRole("tooltip");
    expect(tip).toHaveTextContent(/maybe kindness was the entire point/i);
    expect(tip).toHaveTextContent(/M-0001/);
  });

  it("can paint the same wall as a silent hero backdrop", () => {
    const { container } = render(
      <MonumentCanvas
        decorative
        canvas={canvas}
