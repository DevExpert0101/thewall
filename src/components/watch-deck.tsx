...
import { formatEventInstant, remainingMsFrom } from "@/lib/event/remaining";
...
export function WatchDeck({
  ...
  const displayPhase = phase === "live" && remaining <= 0 ? "finalizing" : phase;
  ...
  const label = phase === "upcoming" ? "Until launch" : phase === "live" ? "Remaining" : "Closed";
      ...
      <header className="watch-chrome">
        <div className="watch-brand">
          ...
          <p className="watch-kicker">
            {phase === "upcoming" ? `Opens ${formatEventInstant(clock.startsAt)}` : meta.label}
          </p>
        </div>
        <Countdown
          targetIso={target}
