...
export function PublishDialog({ open, onOpenChange, enabled, endsAt, serverNow, editionNumber }: Props) {
  ...
  const shownError = closedOut ? closedError : error;
  ...
  useEffect(() => {
    ...
    // Restore a stored checkout only when the dialog opens.
        ...
        >
          ...
          {docked ? (
            <div className="dialog-dock">
              {loadingLine ? (
                <p className="dialog-dock-status" role="status" aria-live="polite">
                  <span className="live-dot" aria-hidden="true" />
                  {loadingLine}
                </p>
              ) : null}
