...
import { assignFinalRanks, compareRising, inFinalHour, selectHiddenGems } from "@/lib/ranking";
...
const seedModeration = new Map<number, "removed" | "approved">();
...
function applyPersisted(raw: PersistedState) {
  ...
  seedModeration.clear();
  for (const [n, status] of raw.seedModeration ?? []) seedModeration.set(n, status);
  extraFires.clear();
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        ops: opsOverride,
        opsAudit,
        seedModeration: [...seedModeration.entries()],
      }),
    );
    try {
...
function assertSimulatedWallWritable(now: Date = new Date()) {
  ...
  if (Date.parse(liveWindow(now).startsAt) > now.getTime()) {
    throw new AppError(ERROR_CODES.EVENT_UPCOMING, "The Wall has not opened yet.", 403);
  }
...
function summaryFromArchive(snapshot: SimulatedArchive): EditionSummary {
  const winning = snapshot.messages.find((message) => message.finalRank === 1) ?? null;
  return {
...
export function simulatedTextAlreadyPublished(text: string): boolean {
  ...
  if (extraWrites.some((row) => !row.removed && normalizeMessage(row.text) === needle)) return true;
  return SEEDS.some((seed) => !seedIsRemoved(seed) && normalizeMessage(seed.text) === needle);
}
...
function withFinalRanks(messages: PublicMessage[]): PublicMessage[] {
  return assignFinalRanks(messages);
}
...
export function simulatedMessageList(
  ...
  const seeds = SEEDS.map((seed) => {
    ...
    const isRemoved = seedIsRemoved(seed);
    return {
      id,
      eventId: SIMULATION_EVENT_ID,
      publicNumber: seed.n,
      text: isRemoved ? ARCHIVAL_REMOVAL_TEXT : seed.text,
      isRemoved,
...
export function getSimulatedMessageById(messageIdValue: string): PublicMessage | null {
  const live = simulatedMessageList().find((row) => row.id === messageIdValue);
  if (live) return live;
  if (archiveSnapshot) {
    const archived = archiveSnapshot.messages.find((row) => row.id === messageIdValue);
    if (archived) return archived;
  }
  for (const edition of sealedEditions) {
    const found = edition.messages.find((row) => row.id === messageIdValue);
    if (found) return found;
  }
  return null;
}

function originalSimulatedText(publicNumber: number): string {
  const seed = SEEDS.find((row) => row.n === publicNumber);
  if (seed) return seed.text;
  const write = extraWrites.find((row) => row.publicNumber === publicNumber);
  return write?.text ?? ARCHIVAL_REMOVAL_TEXT;
}

function patchSimulatedArchiveMessage(messages: PublicMessage[], next: PublicMessage) {
...
export function moderateSimulatedMessage(input: {
  messageId: string;
  action: "remove" | "restore";
}): { publicNumber: number; action: "remove" | "restore" } {
  ensureLoaded();
  const message = getSimulatedMessageById(input.messageId);
  if (!message) {
    throw new AppError(ERROR_CODES.MESSAGE_NOT_FOUND, "Message not found.", 404);
  }
  const remove = input.action === "remove";
  const seed = SEEDS.find((row) => row.n === message.publicNumber);
  if (seed) {
    seedModeration.set(seed.n, remove ? "removed" : "approved");
  } else {
    const write = extraWrites.find((row) => row.publicNumber === message.publicNumber);
    if (!write) {
      throw new AppError(ERROR_CODES.MESSAGE_NOT_FOUND, "Message not found.", 404);
    }
...
export function fulfillSimulatedPayment(input: {
  ...
  if (intent.status === "expired" || Date.parse(intent.expiresAt) <= now.getTime()) {
