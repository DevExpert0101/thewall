...
/** Advance upcoming/live from the clock without writing phase state in an effect. */
export function phaseAfterClock(
  phase: EventPhase,
  startsAt: string,
  endsAt: string,
  now: Date | string | number,
): { phase: EventPhase; remaining: number } {
  if (phase === "archived" || phase === "finalizing") {
    return { phase, remaining: remainingMsFrom(endsAt, now) };
  }
  if (phase === "upcoming" && remainingMsFrom(startsAt, now) > 0) {
    return { phase: "upcoming", remaining: remainingMsFrom(startsAt, now) };
  }
  const remaining = remainingMsFrom(endsAt, now);
  if (remaining <= 0) return { phase: "finalizing", remaining };
  return { phase: "live", remaining };
}
