...
export async function moderateMessage(input: {
  ...
  if (isSimulation() || !hasSupabaseConfig()) {
    const preview = getSimulatedMessageById(input.messageId);
    if (!preview) {
    ...
    assertDangerousConfirm({
      ...
      action: input.action,
      publicNumber: preview.publicNumber,
    });
    const simulated = moderateSimulatedMessage({
      messageId: input.messageId,
      action: input.action,
    });
    const result = { ok: true as const, publicNumber: simulated.publicNumber, action: input.action };
    try {
