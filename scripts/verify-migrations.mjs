...
async function main() {
  ...
  console.log("\nLiving Victor after review removal");
  const EVENT_REVIEW = "b1eebc99-9c0b-4ef8-bb6d-6bb9bd380a11";
  const livingText = "still standing after review";
  const illegalText = "illegal leader for review";
  await db.exec(`
    insert into public.events (id, slug, title, starts_at, ends_at)
    values (
      '${EVENT_REVIEW}',
      'review-wall',
      'REVIEW WALL',
    ...
    values ('${EVENT_REVIEW}', 3, 2, 130)
    on conflict (event_id) do update
      set next_message_number = 3, total_messages = 2, total_reactions = 130;
  `);
  const reviewIllegalIntent = await db.query(
    `
    insert into public.payment_intents (
      event_id, anonymous_user_id, message_text, message_hash,
      amount, currency, network, recipient_wallet, status, expires_at
    ) values (
      $1, gen_random_uuid(), $2, $3, 1, 'USDC', 'base', $4, 'created', now() + interval '15 minutes'
    ) returning id
    `,
    [EVENT_REVIEW, illegalText, sha256Hex(illegalText), TREASURY],
  );
  const reviewLivingIntent = await db.query(
    `
    insert into public.payment_intents (
      event_id, anonymous_user_id, message_text, message_hash,
      amount, currency, network, recipient_wallet, status, expires_at
    ...
    `,
    [EVENT_REVIEW, livingText, sha256Hex(livingText), TREASURY],
  );
  await db.exec(`
    insert into public.messages (
      event_id, public_number, payment_intent_id, text, text_hash, reaction_count, published_at
    ) values
      (
        '${EVENT_REVIEW}', 1, '${reviewIllegalIntent.rows[0].id}',
        '${illegalText}', '${sha256Hex(illegalText)}', 90, now() - interval '30 minutes'
      ),
      (
        '${EVENT_REVIEW}', 2, '${reviewLivingIntent.rows[0].id}',
        '${livingText}', '${sha256Hex(livingText)}', 40, now() - interval '20 minutes'
      );
    update public.messages
      set removed_at = now(), removal_reason_code = 'abuse', moderation_status = 'removed'
      where event_id = '${EVENT_REVIEW}' and public_number = 1;
    select public.finalize_event_rankings('${EVENT_REVIEW}'::uuid);
  `);
  const reviewRanks = await db.query(`
    select public_number, final_rank
    from public.messages
    where event_id = '${EVENT_REVIEW}'
    order by public_number
  `);
  assert(reviewRanks.rows[0].final_rank == null, "removed leader has no final rank at Finish");
  assert(reviewRanks.rows[1].final_rank === 1, "next living sentence receives final rank #1");
  const reviewMonument = await db.query(`
    select sentence_snapshot, original_public_number
    from public.monument_entries
    where event_id = '${EVENT_REVIEW}'
  `);
  assert(reviewMonument.rows.length === 1, "Finish after removal still carves one Monument sentence");
  assert(reviewMonument.rows[0].original_public_number === 2, "Victor is the living runner-up");
  assert(reviewMonument.rows[0].sentence_snapshot === livingText, "Monument stores the living sentence");
